...
def blogs():
    ...
    posts=db.session.query(Post.author_id,Post.title,Post.body,Post.created,Post.id,Post.filename,Employee.username).\
...
def uploads(files,key):
    BLOG_IMG_FOLDER= current_app.config['BLOG_IMG_FOLDER']
    # filename = secure_filename(key+'_'+files.filename)
    filename = key+'_'+files.filename 
    filesv= os.path.join(BLOG_IMG_FOLDER, filename)
    files.save(filesv)
    return filename
...
def create():
    ...
    if form.validate_on_submit():
        ...
        if files:
           filenm=uploads(files,now.strftime("%Y-%m-%d"))
           posts.filename=filenm
    ...
    return render_template('blog/create.html', form=form, title='Register blog',file=None)
...
def update(id):
    ...
    return render_template('blog/create.html', form=form, title='Update blog',file=post.filename)
...
def detail(id):
    post = Post.query.get_or_404(id)  #from table
    row= post.body.count('\n')
    print('body len==',len(post.body), row)
    return render_template('blog/detail.html', post=post, title='Blog detials',row=row)
...
def delete(id):
    ...
    if post.filename:
       BLOG_IMG_FOLDER= current_app.config['BLOG_IMG_FOLDER']
       filenm= os.path.join(BLOG_IMG_FOLDER, post.filename)
       os.remove(filenm)
    return redirect(url_for('blog.blogs'))


@blog.route('/blog/filedown/<string:filename>', methods=['GET', 'POST'])
def download_file(filename):
    BLOG_IMG_FOLDER= current_app.config['BLOG_IMG_FOLDER']
    print(BLOG_IMG_FOLDER,filename)
    return send_from_directory(directory=BLOG_IMG_FOLDER,filename=filename)
