...
#1. 메트로 코드 분리.  2. 직영,자율구분값 (colNm에 생성)
def BrhConv(dfC,colNm):
    ...
    #---직영,자율 구분  => 속도느리면 df만들어 join할 것
    dfC.loc[:,colNm] = dfC.지점.map(lambda x: '자율'  if x in GROUP2 else ('직영' if x in GROUP1 else '오류'))
    return dfC

'''
str='엠피아이리치'
if str in GROUP1: print('직영')
elif str in GROUP2: print('자율')
else: print('err')
'''